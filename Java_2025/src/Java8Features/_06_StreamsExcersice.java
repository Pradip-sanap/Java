...
public class _06_StreamsExcersice {
	...
	public static void main(String[] args) {
		...
		List<Integer> newStream2 = nums.stream()
			.map(e -> e * 5).collect(Collectors.toList());
		...
		List<String> upperCaseName = names.stream().map(name -> name.toUpperCase()).collect(Collectors.toList());
		System.out.println(upperCaseName);
	
		List<String> find_hit = names.stream().filter(name -> name.contains("hit")).collect(Collectors.toList());
